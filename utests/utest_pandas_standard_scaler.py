...
class TestPandasStandardScalerv0(unittest.TestCase):
    ...
    def test_output_types(self):
        ...
        output = prep.run(self.data)
        self.assertEqual(type(output), pd.DataFrame, msg='Output not DataFrame when inplace==False')
    ...
    def test_axis_zero(self):
        ...
        self.assertTrue(np.allclose(mean, np.zeros_like(mean)), msg='Column mean not equal to zero')
        self.assertTrue(np.allclose(var, np.ones_like(var)), msg='Column variance not equal to one')
    ...
    def test_zero_variance(self):
        ...
        theory_var[-1] = 0
        self.assertTrue(np.allclose(var, theory_var), msg='Scaled variance is incorrect')
    ...
    def test_multi_run(self):
        ...
        self.assertTrue((saved_mean == prep.mean).all(), msg='Mean has changed after second run()')
        self.assertTrue((saved_scale == prep.scale).all(), msg='Scale has changed after second run()')

        # Mean in data+5 after scaling should be 5 / scale
        self.assertTrue(np.allclose(scaled_data2.mean(), 5/prep.scale), msg='Mean of second run() is incorrect.')
    ...
    def test_save_load(self):
        ...
        try:
            ...
            self.assertTrue(np.allclose(new_scaled_data, scaled_data), msg='Scaled data after load() does not match')
        finally:
