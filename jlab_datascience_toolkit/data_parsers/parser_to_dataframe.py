...
pandas_read_functions = dict(
    csv=pd.read_csv, feather=pd.read_feather, json=pd.read_json, pickle=pd.read_pickle
)
...
class Parser2DataFrame(JDSTDataParser):
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        for file in self.config["filepaths"]:
            ...
            data = self.read_function(file, **self.config["read_kwargs"])
        ...
        output = pd.concat(data_list, **self.config["concat_kwargs"])
